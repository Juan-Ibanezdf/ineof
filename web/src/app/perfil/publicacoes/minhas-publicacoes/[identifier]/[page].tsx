...
const PublicacaoDetalhes: React.FC = () => {
  ...
  const { register, handleSubmit, setValue, watch, formState: { errors } } = useForm<Publicacao>();
  ...
  useEffect(() => {
    ...
    const fetchPublicacaoDetails = async () => {
      ...
      try {
        const response = await api.get(`/api/user/publications/${identifier}/${slug}`);
  ...
  const onSubmit = async (formData: Publicacao) => {
    const api = getAPIClient();
    try {
      await api.put(`/api/user/publications/${identifier}/${slug}`, formData);
  ...
  const handleDelete = async () => {
    const api = getAPIClient();
    try {
      await api.delete(`/api/user/publicacao/${publicacao?.idPublicacao}`);
  ...
  return (
    <>
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        ...
        {!isEditing ? (
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 gap-6">
            <div>
              ...
              {errors.titulo && <span className="text-red-500"></span>}
            </div>
            <div>
              ...
              {errors.resumo && <span className="text-red-500"></span>}
            </div>
        ...
        <Link href="/perfil/publicacoes/minhas-publicacoes"><span className="mt-4 inline-block text-indigo-600 hover:text-indigo-500">Voltar para Minhas Publicações</span></Link>
      </main>
      <Footer />
    </>
  );
