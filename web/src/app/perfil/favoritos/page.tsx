  ...
  const fetchFavoritos = useCallback(async () => {
    try {
      ...
      const favoritosMapeados = response.data.map((favorito: any) => ({
        id_favoritos: favorito.id_favoritos,  // Corrigido para o nome correto
      ...
      <div key={favorito.id_favoritos} className="py-4 border-b flex justify-between items-center">
        <div>
          ...
          <Link href={`/publicacoes/publicacao/${favorito.identifier}/${favorito.slug}`}>
            <span className="inline-block mt-4 text-blue-500 hover:text-blue-700">Ver publicação</span>
          </Link>
        </div>

        <button
          onClick={() => removerFavorito(favorito.id_favoritos)}
  ...
  return (
    <Layout>
      <div className="p-8 bg-gray-100 px-52">
        <h2 className="text-3xl font-bold mb-6 text-blue-ineof">Meus Favoritos</h2>
        ...
        <div className="flex items-center mb-6">
          ...
          <button onClick={filtrarFavoritos} className="bg-blue-500 text-white p-2 rounded">
            Buscar
          </button>
